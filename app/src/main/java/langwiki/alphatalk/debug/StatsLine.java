...
public class StatsLine {
     ...
     */
    public StatsLine(String lineTag) {
        mLineTag = lineTag;
        mLastPrintTimeMS = -1;
    ...
    protected List<ColumnBase<? extends Number>> mColumns =
            new ArrayList<ColumnBase<? extends Number>>();

    /**
     * Adds a column into the statistic line.
     * @param column The {@link ColumnBase} object.
     */
    public void addColumn(ColumnBase<? extends Number> column) {
        mColumns.add(column);
    }
    ...
    public synchronized void startLine() {
        for (ColumnBase<? extends Number> col : mColumns) {
            col.onStartLine();
        }
    }
     ...
     */
    public static abstract class ColumnBase<T> {
     ...
     */
    public static class StandardColumn<T extends Number> extends ColumnBase<T> {
         ...
         */
        public StandardColumn(String name) {
            super(name);
        }
        ...
        public synchronized Object getStat() {
            switch (mData.size()) {
                ...
                default: {
                    Stats.DescriptiveResult res = Stats.computeDescriptive(mData);
